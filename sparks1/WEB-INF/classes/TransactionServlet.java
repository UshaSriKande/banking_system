...
ResultSet rs = stm.executeQuery("select balance from cus2 where name='"+usr1+"'");

if(rs.next()){
String n = rs.getString(1);
int balance1 = Integer.parseInt(n);
if(balance1 - amount > 0){
String q = "update cus2 set balance = balance +'"+amount+"' where name='"+usr+"'";
int x = stm.executeUpdate(q);
String q1 = "update cus2 set balance = balance - '"+amount+"' where name='"+usr1+"'";
...
out.println("<center><h1>Transaction Succesfull...</h1></center>");
...
out.println("<center><h1>Transaction Failed...</h1></center>");
